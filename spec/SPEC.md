...
| バージョン | 変更概要 |
|-----------|---------|
| 1.0.0 (現在) | `Registry` をインスタンスファースト設計に再構築 (`Registry.global()` / `Registry.named()` / `new Registry()`)。旧 static API は廃止し、`Singletons` ファサード経由でのみ後方互換を維持する **破壊的変更**。Javadoc を全面強化。 |
| 0.9.2 | `PropStack`、`TypedKey`、`Registry`、`RegistryKey`、`KeyHolder` 安定化。`ApplicationProperties`・`Singletons` を後方互換エイリアスとして保持。 |
...
graph TD
    PS[PropStack]
    SRC["List&lt;PropertySource&gt; sources (ArrayList)"]
    S0["sources[0]: HashMap&lt;String,String&gt;<br/>first — set() 書き込み先"]
    S1["sources[1]: System.getProperty()<br/>SystemProperties"]
    S2["sources[2]: System.getenv()<br/>EnvironmentVariables"]
    S3["sources[3]: Path → Properties<br/>fromPath (ユーザーホーム)"]
    SN["sources[n]: ClassLoader → Properties<br/>fromClasspath"]
...
```mermaid
graph TD
    G["Registry.global()<br/>(static GLOBAL)"]
    N["Registry.named(String)<br/>(static ConcurrentHashMap&lt;String, Registry&gt; NAMED)"]
    INST["new Registry()<br/>(独立インスタンス)"]
    MAP["ConcurrentHashMap&lt;String, Object&gt; store<br/>各インスタンスごとに独立"]
    K1["&quot;com.example.DataSource&quot;<br/>→ DataSource インスタンス"]
    K2["&quot;com.example.DataSource#PROD&quot;<br/>→ DataSource インスタンス (named key)"]
    K3["&quot;myCustomService&quot;<br/>→ Object"]
...
`store` は `Registry` の **インスタンスフィールド** であるため、各 `Registry` は独立した状態を持つ。`Registry.global()` が返すシングルトンと `Registry.named(String)` で得る名前付き共有インスタンスのみ、static フィールドから到達可能で JVM ライフタイムまで保持される。明示的に `new Registry()` で作ったインスタンスはガベージコレクション可能。
...
classDiagram
    ...
    class Registry {
        ...
        -Registry GLOBAL$
        -ConcurrentHashMap~String,Registry~ NAMED$
        +global()$ Registry
        +named(String name)$ Registry
        +Registry()
        +get(Class~T~ clazz) T
...
flowchart TD
    ...
    F --> G{value.isPresent<br/>AND not blank?}
...
0.10 以降は **インスタンスファースト設計**。`Registry` はインスタンス化可能で、各インスタンスが独立したコンポーネントマップを持つ。プロセスワイドな共有が必要な場合は `Registry.global()`、モジュール単位の共有名前空間が欲しい場合は `Registry.named(String)` を使う。

旧 `Registry.put(...)` 形の static 呼び出しは廃止された。後方互換が必要な既存コードは `Singletons` ファサード(§7.2)を使うか、`Registry.global().put(...)` のように global() インスタンス経由に書き換える。

```java
public final class Registry {

    // ---- ファクトリ ----
    public static Registry global()                // プロセスワイドな共有インスタンス
    public static Registry named(String name)      // 名前付き共有インスタンス(同名は同一インスタンス)

    public Registry()                              // 独立した空インスタンス
...
| `Singletons` | `Registry.global()` | クラス指定のみ対応（RegistryKey 非対応） |

`ApplicationProperties` は既存コードの変更なしに継続利用できる。`Singletons` も継続して使えるが、内部的にはすべて `Registry.global()` に委譲される。新規コードでは `PropStack` / `Registry`(インスタンス、または `Registry.global()` / `Registry.named()`)を使う。
...
```mermaid
graph LR
    S0["[0] props.set()<br/>プログラマティック<br/>最高優先度"]
    S1["[1] -DKEY=val<br/>JVM システムプロパティ"]
    S2["[2] KEY=val<br/>OS 環境変数"]
    S3["[3] ~/&lt;appName&gt;/application.properties<br/>ユーザーホームファイル"]
    S4["[4] classpath:application.properties<br/>クラスパスデフォルト<br/>最低優先度"]

    S0 -->|優先度 高 → 低| S1 --> S2 --> S3 --> S4
```
...
```gradle
implementation 'org.unlaxer:propstack:1.0.0'
```
...
classDiagram
    class PropertySource {
        <<interface>>
        +getRawValue(key) Optional~String~
        +set(key, value) PropertySource
        +keys() Set~String~
        +get(key) Optional~String~
        +get(keys) Optional~String~
        +valueEffectors() List~UnaryOperator~
    }
    ...
    class TypedKey~T~ {
        ...
        +description String
        +sensitive boolean
        +defaultsTo(V) TypedKey~V~
        +describedAs(String) TypedKey~T~
    }
    ...
    class Registry {
        -instances$ ConcurrentHashMap~String,Object~
    }
    ...
    PropStack ..|> PropertySource : implements
    PropStack "1" o-- "0..*" PropertySource : sources
    KeyHolder --> TypedKey : holds
    ApplicationProperties --|> PropStack : extends
    Singletons ..> Registry : delegates
```
...
*PropStack SPEC.md — opaopa6969/propstack v1.0.0*  
*生成日: 2026-05-14*
